...
const statusBadge = (status: string) => {
  const map: Record<string, { bg: string; text: string; border: string; icon: any }> = {
    ...
    lost: { bg: 'bg-slate-100', text: 'text-slate-600', border: 'border-slate-200', icon: Ban },
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Library Management</h1>
          <p className="text-sm text-slate-500 mt-1">Manage books, issuing, returns, and library reports.</p>
        </div>
        <div className="flex items-center gap-3">
          {activeTab === 'catalog' && (
            <>
              <button onClick={() => { setFilters(p => ({ ...p, isArchived: !p.isArchived })); }}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors border shadow-sm ${filters.isArchived ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
                <Archive size={16} className="inline mr-1.5" />{filters.isArchived ? 'Archived' : 'Active'}
              </button>
              <button onClick={() => exportBooksToCSV(books)} className="bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 px-4 py-2 rounded-xl text-sm font-medium transition-colors shadow-sm flex items-center gap-2">
                <Download size={16} /> Export
              </button>
              <button onClick={() => openBookModal('create')} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-medium shadow-md transition-all flex items-center gap-2">
                <Plus size={16} /> Add Book
              </button>
            </>
          )}
          {activeTab === 'transactions' && (
            <button onClick={() => openIssueModal()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-medium shadow-md transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-2xl font-bold text-slate-800 mt-1">{inventory.totalBooks}</p>
          <p className="text-xs text-slate-400 mt-0.5">{inventory.totalCopies} total copies</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <p className="text-2xl font-bold text-blue-600 mt-1">{inventory.issuedCopies}</p>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-sm font-medium text-slate-500">Overdue Returns</p>
          <p className={`text-2xl font-bold mt-1 ${overdueCount > 0 ? 'text-rose-600' : 'text-slate-800'}`}>{overdueCount}</p>
        </div>
      </div>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50/50">
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              {tab.label}
              {activeTab === tab.id && <motion.div layoutId="libraryTabIndicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600" />}
            </button>
          ))}
        </div>
                      ...
                      {filteredBooks.map(book => (
                        <tr key={book.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-4">
                            ...
                            <div className="text-xs text-slate-400 mt-0.5">{book.id}</div>
                          </td>
                          ...
                          <td className="px-6 py-4 font-medium text-slate-800">{book.totalCopies}</td>
                          <td className="px-6 py-4">
                            <span className={`font-medium ${book.availableCopies > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                              {book.availableCopies}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-slate-500">{book.shelfLocation || '—'}</td>
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-0.5">
                              <button onClick={() => openBookModal('view', book)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View"><Eye size={15} /></button>
                              <button onClick={() => openBookModal('edit', book)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit"><Pencil size={15} /></button>
                              {!filters.isArchived && book.availableCopies > 0 && (
                                <button onClick={() => openIssueModal(book.id)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Issue"><BookMarked size={15} /></button>
                              )}
                              {!filters.isArchived ? (
                                <button onClick={() => handleBookAction('archive', book.id)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Archive"><Archive size={15} /></button>
                              ) : (
                                <button onClick={() => handleBookAction('restore', book.id)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Restore"><Undo2 size={15} /></button>
                              )}
                              <button onClick={() => handleBookAction('delete', book.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete"><Trash2 size={15} /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
                      {filteredBooks.length === 0 && (
                        <tr><td colSpan={8} className="px-6 py-12 text-center text-sm text-slate-400">{filters.isArchived ? 'No archived books.' : 'No books in catalog. Add your first book.'}</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                  ...
                  <div className="flex gap-2">
                    {['all', 'issued', 'returned', 'overdue', 'lost'].map(s => (
                      <button key={s} onClick={() => { /* could filter */ }}
                        className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-colors ${s === 'all' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {borrowRecords.sort((a, b) => new Date(b.issueDate).getTime() - new Date(a.issueDate).getTime()).map(record => (
                        <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-4 font-medium text-slate-800">{record.bookTitle}</td>
                          <td className="px-6 py-4">
                            <div className="font-medium text-slate-800">{record.userName}</div>
                            <div className="text-xs text-slate-400">{record.userId}</div>
                          </td>
                          <td className="px-6 py-4 text-slate-600 capitalize">{record.userRole}</td>
                          <td className="px-6 py-4 text-slate-500">{new Date(record.issueDate).toLocaleDateString()}</td>
                          <td className={`px-6 py-4 font-medium ${record.status === 'overdue' ? 'text-rose-600' : 'text-slate-600'}`}>
                            {new Date(record.dueDate).toLocaleDateString()}
                          </td>
                          <td className="px-6 py-4 text-slate-500">{record.returnDate ? new Date(record.returnDate).toLocaleDateString() : '—'}</td>
                          <td className="px-6 py-4">{statusBadge(record.status)}</td>
                          <td className="px-6 py-4">
                            {record.fine > 0 ? (
                              <span className={`font-medium ${record.finePaid ? 'text-slate-400' : 'text-rose-600'}`}>
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-0.5">
                              {record.status === 'issued' && (
                                <>
                                  <button onClick={() => openReturnModal(record)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Return"><RotateCcw size={15} /></button>
                                  <button onClick={() => { if (renewBook(record.id)) refreshBorrows(); else alert('Cannot renew (max 2 renewals).'); }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Renew"><RotateCcw size={15} /></button>
                                  <button onClick={() => { markAsLost(record.id); refreshBooks(); refreshBorrows(); }} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Mark Lost"><Ban size={15} /></button>
                                </>
                              )}
                              {record.status === 'overdue' && (
                                <>
                                  ...
                                  {!record.finePaid && record.fine > 0 && (
                                    <button onClick={() => { payFine(record.id); refreshBorrows(); }} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Pay Fine"><DollarSign size={15} /></button>
                                  )}
                                </>
                              )}
                            </div>
                      ...
                      {borrowRecords.length === 0 && (
                        <tr><td colSpan={9} className="px-6 py-12 text-center text-sm text-slate-400">No borrow records yet.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                ...
                <div className="flex gap-3 mb-6">
                  {[
                    ...
                    <button key={r.id} onClick={() => setReportType(r.id as any)}
                      className={`px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${reportType === r.id ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
                      {r.label}
                    </button>
                  ))}
                </div>
                ...
                {reportType === 'inventory' && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                      ...
                      <p className="text-2xl font-bold text-slate-800 mt-1">{inventory.totalCopies}</p>
                    </div>
                    <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                ...
                {reportType === 'overdue' && (
                  <div className="overflow-x-auto">
                    {overdueRecords.length === 0 ? (
                      <div className="text-center py-12 text-sm text-slate-400">No overdue books. All books have been returned on time.</div>
                    ) : (
                      <table className="w-full text-sm text-left">
                          ...
                          {overdueRecords.map(r => {
                            ...
                            return (
                              <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-6 py-4 font-medium text-slate-800">{r.bookTitle}</td>
                                <td className="px-6 py-4">{r.userName}</td>
                                <td className="px-6 py-4 text-slate-500">{new Date(r.issueDate).toLocaleDateString()}</td>
                ...
                {reportType === 'popular' && (
                  <div className="overflow-x-auto">
                    {popularBooks.length === 0 ? (
                      <div className="text-center py-12 text-sm text-slate-400">No borrowing data available yet.</div>
                    ) : (
                      <table className="w-full text-sm text-left">
                        <thead className="text-xs text-slate-500 uppercase bg-slate-50 rounded-xl">
                          <tr>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {popularBooks.map((b, i) => (
                            <tr key={b.bookId} className="hover:bg-slate-50/50 transition-colors">
                              <td className="px-6 py-4 text-slate-500 font-medium">{i + 1}</td>
                              <td className="px-6 py-4 font-medium text-slate-800">{b.bookTitle}</td>
                              <td className="px-6 py-4"><span className="font-bold text-blue-600">{b.borrowCount}x</span></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
      ...
      <AnimatePresence>
        {bookModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={closeBookModal} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl z-10 max-h-[90vh] overflow-y-auto">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10 rounded-t-2xl">
                ...
                <button onClick={closeBookModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><X size={20} /></button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Title <span className="text-rose-500">*</span></label>
                    <input type="text" value={bookModal.data.title || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, title: e.target.value } }))}
                      ...
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white read-only:bg-slate-50" /></div>
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Author <span className="text-rose-500">*</span></label>
                    <input type="text" value={bookModal.data.author || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, author: e.target.value } }))}
                      readOnly={bookModal.mode === 'view'}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white read-only:bg-slate-50" /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">ISBN <span className="text-rose-500">*</span></label>
                    <input type="text" value={bookModal.data.isbn || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, isbn: e.target.value } }))}
                      readOnly={bookModal.mode === 'view'}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white read-only:bg-slate-50" /></div>
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Publisher</label>
                    <input type="text" value={bookModal.data.publisher || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, publisher: e.target.value } }))}
                      readOnly={bookModal.mode === 'view'}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white read-only:bg-slate-50" /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Edition</label>
                  ...
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Language</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Subject</label>
                    <input type="text" value={bookModal.data.subject || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, subject: e.target.value } }))}
                      readOnly={bookModal.mode === 'view'}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white read-only:bg-slate-50" /></div>
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Publication Year</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Total Copies <span className="text-rose-500">*</span></label>
                    <input type="number" value={bookModal.data.totalCopies || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, totalCopies: Number(e.target.value) } }))}
                      readOnly={bookModal.mode === 'view'}
                      className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white read-only:bg-slate-50" /></div>
                  <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Shelf Location</label>
                ...
                {bookModal.mode !== 'view' && (
                  <div>
                    <label className="text-sm font-medium text-slate-700 block mb-2">Associated Classes</label>
                    <div className="flex flex-wrap gap-1.5">
                      {classes.map(c => {
                        const checked = (bookModal.data.classIds || []).includes(c.id);
                        return (
                          <button key={c.id} type="button" onClick={() => toggleBookClass(c.id)}
                            className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-colors ${checked ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                            {c.name}
                          </button>
                        );
                      })}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {bookModal.data.classIds.map(cid => {
                        ...
                        return cls ? (
                          <span key={cid} className="px-2.5 py-1 rounded-lg text-xs font-medium border bg-indigo-50 text-indigo-700 border-indigo-200">{cls.name}</span>
                        ) : null;
                      })}
                    </div>
                  </div>
                ...
                <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Description</label>
                  <textarea value={bookModal.data.description || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, description: e.target.value } }))}
                    readOnly={bookModal.mode === 'view'}
                    className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 min-h-[60px] bg-white read-only:bg-slate-50" /></div>

                {bookModal.mode !== 'view' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Thumbnail URL</label>
                      <input type="text" value={bookModal.data.thumbnail || ''} onChange={e => setBookModal(p => ({ ...p, data: { ...p.data, thumbnail: e.target.value } }))}
                        className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white" /></div>
                    <div className="space-y-1"><label className="text-sm font-medium text-slate-700">PDF URL (digital resource)</label>
                ...
                {bookModal.mode === 'view' && (
                  <div className="text-sm bg-slate-50 rounded-xl p-3 space-y-1">
                    <div className="flex justify-between"><span className="text-slate-500">Copies:</span><span className="font-medium text-slate-700">{bookModal.data.availableCopies} / {bookModal.data.totalCopies} available</span></div>
                    {bookModal.data.pdfUrl && <div className="flex justify-between"><span className="text-slate-500">Digital:</span><a href={bookModal.data.pdfUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Open PDF</a></div>}
                  </div>
                )}
              </div>
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
                <button onClick={closeBookModal} className="flex-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-medium transition-colors">
                  {bookModal.mode === 'view' ? 'Close' : 'Cancel'}
                </button>
                {bookModal.mode !== 'view' && (
                  <button onClick={handleSaveBook} disabled={bookSaving}
                    className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium text-white transition-colors flex items-center justify-center gap-2 ${bookSaving ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700 shadow-sm'}`}>
      ...
      <AnimatePresence>
        {issueModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={closeIssueModal} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg z-10">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 text-lg">Issue a Book</h3>
                <button onClick={closeIssueModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><X size={20} /></button>
              </div>
              <div className="p-6 space-y-4">
                <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Book <span className="text-rose-500">*</span></label>
                ...
                <div className="space-y-1"><label className="text-sm font-medium text-slate-700">User Role</label>
                ...
                <div className="space-y-1"><label className="text-sm font-medium text-slate-700">User <span className="text-rose-500">*</span></label>
                ...
                <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Due Date <span className="text-rose-500">*</span></label>
              ...
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
                <button onClick={closeIssueModal} className="flex-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-medium transition-colors">Cancel</button>
                <button onClick={handleIssue} disabled={issueSaving}
                  className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium text-white transition-colors flex items-center justify-center gap-2 ${issueSaving ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700 shadow-sm'}`}>
      ...
      <AnimatePresence>
        {returnModal.open && returnModal.record && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={closeReturnModal} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md z-10">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 text-lg">Return Book</h3>
                <button onClick={closeReturnModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><X size={20} /></button>
              </div>
              <div className="p-6 space-y-4">
                <div className="text-sm bg-slate-50 rounded-xl p-3 space-y-1">
                  <div className="flex justify-between"><span className="text-slate-500">Book:</span><span className="font-medium text-slate-700">{returnModal.record.bookTitle}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Borrower:</span><span className="font-medium text-slate-700">{returnModal.record.userName}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">Due Date:</span><span className="font-medium text-slate-700">{new Date(returnModal.record.dueDate).toLocaleDateString()}</span></div>
                  {(() => {
                    const fine = calculateFine(returnModal.record.dueDate, returnDate || new Date().toISOString().split('T')[0]);
                    return fine > 0 ? (
                      <div className="flex justify-between"><span className="text-slate-500">Fine:</span><span className="font-bold text-rose-600">৳{fine}</span></div>
                    ) : null;
                  })()}
                </div>
                <div className="space-y-1"><label className="text-sm font-medium text-slate-700">Return Date</label>
                    <input type="date" value={returnDate} onChange={e => setReturnDate(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" /></div>
              </div>
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
                <button onClick={closeReturnModal} className="flex-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-medium transition-colors">Cancel</button>
