...
export default function ExamManagementPage() {
  ...
  const statusBadge = (status: ExamStatus) => {
    const map: Record<ExamStatus, { bg: string; text: string; border: string; icon: any }> = {
      draft: { bg: 'bg-slate-50', text: 'text-slate-700', border: 'border-slate-200', icon: Settings },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Exam & Result Management</h1>
          <p className="text-sm text-slate-500 mt-1">Manage examinations, schedules, marks entry, results, and report cards.</p>
        </div>
        <div className="flex items-center gap-3">
          {activeTab === 'exams' && (
            <button onClick={() => openExamModal('add')} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-medium shadow-md shadow-blue-500/20 transition-all flex items-center gap-2">
        ...
        <div className="flex border-b border-slate-200 bg-slate-50/50 overflow-x-auto custom-scrollbar">
          {TABS.map(tab => (
            ...
            >
              ...
              {tab.label}
              {activeTab === tab.id && (
                <motion.div layoutId="examTabIndicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-600" />
              )}
            </button>
          ))}
        </div>
          ...
          <AnimatePresence mode="wait">
                      ...
                      {filteredExams.map(exam => (
                        <tr key={exam.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-4 font-semibold text-slate-800">
                            <div>{exam.name}</div>
                            <div className="text-xs text-slate-500 font-normal mt-0.5">{exam.id}</div>
                          </td>
                          <td className="px-6 py-4 text-slate-600">{exam.examType}</td>
                          <td className="px-6 py-4 text-slate-600">{exam.academicYear}</td>
                          <td className="px-6 py-4 text-slate-600">{exam.term}</td>
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center justify-end gap-1">
                              <button onClick={() => openExamModal('edit', exam)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit"><Pencil size={16} /></button>
                              {exam.status !== 'archived' && exam.status !== 'published' && (
                                <button onClick={() => handlePublishExam(exam.id)} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Publish"><Share2 size={16} /></button>
                              )}
                              {exam.status !== 'archived' && (
                                <button onClick={() => handleArchiveExam(exam.id)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Archive"><Archive size={16} /></button>
                              )}
                              <button onClick={() => setConfirmDelete({ type: 'exam', id: exam.id, label: exam.name })} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete"><Trash2 size={16} /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
                      {filteredExams.length === 0 && (
                        <tr><td colSpan={7} className="px-6 py-12 text-center text-sm text-slate-400">No exams found. Click "Create Exam" to begin.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              <motion.div key="schedule" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                  <select value={selectedExamForSchedule} onChange={e => setSelectedExamForSchedule(e.target.value)}
                    className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-full md:w-72">
                  ...
                  {selectedExamForSchedule && (
                    <button onClick={() => openScheduleModal('add')} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-medium shadow-sm transition-all flex items-center gap-2">
                ...
                {!selectedExamForSchedule ? (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                    ...
                    <h3 className="text-lg font-semibold text-slate-700 mb-1">Exam Schedule</h3>
                    <p className="text-sm max-w-sm text-center">Select an exam above to manage its schedule/routine.</p>
                  </div>
                ) : filteredSchedules.length === 0 ? (
                  <div className="text-center py-12 text-sm text-slate-400">No schedules defined for this exam. Click "Add Schedule" to create one.</div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                      <thead className="text-xs text-slate-500 uppercase bg-slate-50 rounded-xl">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {filteredSchedules.map(sch => (
                          <tr key={sch.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-6 py-4 font-semibold text-slate-800">{new Date(sch.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</td>
                            <td className="px-6 py-4">
                              <span className="font-medium text-slate-800">{sch.subjectName}</span>
                            ...
                            <td className="px-6 py-4 text-slate-600">{sch.startTime} – {sch.endTime}</td>
                            <td className="px-6 py-4 text-slate-600">{sch.room || '—'}</td>
                            <td className="px-6 py-4 text-slate-600">{sch.invigilator || '—'}</td>
                            <td className="px-6 py-4 text-slate-600">{sch.passMarks}/{sch.fullMarks}</td>
                            <td className="px-6 py-4">
                              <div className="flex items-center justify-end gap-1">
                                <button onClick={() => openScheduleModal('edit', sch)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit"><Pencil size={16} /></button>
                                <button onClick={() => { if (confirm(`Delete schedule for ${sch.subjectName}?`)) handleDeleteSchedule(sch.id); }} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete"><Trash2 size={16} /></button>
                              </div>
                            </td>
                          </tr>
                        ))}
              ...
              <motion.div key="marks" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <div className="flex flex-col md:flex-row gap-4 mb-4">
                  <select value={marksExamId} onChange={e => { setMarksExamId(e.target.value); setMarksScheduleId(''); setMarksClassFilter(''); setMarksSectionFilter(''); }}
                    className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-full md:w-64">
                  ...
                  <>
                    <div className="flex flex-col md:flex-row gap-3 mb-4 items-start md:items-center">
                      <div className="text-sm text-slate-600 font-medium">
                        Subject: <span className="text-slate-800">{marksSelectedSchedule.subjectName}</span>
                        &nbsp;|&nbsp; Full Marks: <span className="text-slate-800">{marksSelectedSchedule.fullMarks}</span>
                        &nbsp;|&nbsp; Pass Marks: <span className="text-slate-800">{marksSelectedSchedule.passMarks}</span>
                      </div>
                      <div className="flex gap-2 ml-auto">
                        <select value={marksClassFilter} onChange={e => setMarksClassFilter(e.target.value)}
                          className="px-3 py-1.5 border border-slate-200 rounded-xl text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                        ...
                        <div className="relative">
                          ...
                          <input type="text" placeholder="Search student..." value={marksSearch} onChange={e => setMarksSearch(e.target.value)}
                            className="pl-8 pr-3 py-1.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-48" />
                        </div>
                        <button onClick={() => setShowBulkEntry(!showBulkEntry)} className="px-3 py-1.5 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl text-sm font-medium transition-colors flex items-center gap-1.5">
                          <Upload size={14} /> Bulk
                        </button>
                      </div>
                    </div>
                    ...
                    {showBulkEntry && (
                      <div className="flex items-center gap-3 mb-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
                        <span className="text-sm font-medium text-slate-700">Set all marks to:</span>
                        <input type="number" value={bulkMarksValue} onChange={e => setBulkMarksValue(e.target.value)} placeholder={`0-${marksSelectedSchedule.fullMarks}`}
                          className="w-24 px-3 py-1.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                        <button onClick={handleBulkSetMarks} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium transition-colors">Apply</button>
                        <button onClick={() => setShowBulkEntry(false)} className="px-3 py-1.5 text-slate-500 hover:text-slate-700 text-sm">Cancel</button>
                      </div>
                    )}
                          ...
                          {marksStudents.map(s => {
                            ...
                            return (
                              <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-4 py-3 text-slate-600 font-medium">{s.roll}</td>
                                ...
                                <td className="px-4 py-3">
                                  <input type="number" value={entry.obtained} onChange={e => handleMarksChange(s.id, 'obtained', e.target.value)}
                                    ...
                                    className={`w-20 px-2 py-1 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${entry.isAbsent ? 'bg-slate-100 text-slate-400' : isValid ? 'border-slate-200' : 'border-rose-300 bg-rose-50/30'}`}
                                ...
                                <td className="px-4 py-3">
                                  {entry.obtained !== '' && isValid && !entry.isAbsent && (
                                    <span className="text-sm font-medium text-slate-700">{grade} ({gradePoint.toFixed(2)})</span>
                                  )}
                                </td>
                                <td className="px-4 py-3">
                                  <button onClick={() => handleMarksAbsentToggle(s.id)}
                                    className={`w-6 h-6 rounded border flex items-center justify-center transition-colors ${entry.isAbsent ? 'bg-rose-100 border-rose-300 text-rose-600' : 'border-slate-200 text-slate-300 hover:border-slate-300'}`}>
                                    {entry.isAbsent && <X size={12} />}
                                  </button>
                                </td>
                                <td className="px-4 py-3">
                                  <input type="text" value={entry.remarks} onChange={e => handleMarksChange(s.id, 'remarks', e.target.value)}
                                    placeholder="—" className="w-full max-w-[120px] px-2 py-1 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    <div className="flex justify-end mt-4">
                      <button onClick={handleSaveMarks}
                        className="px-6 py-2 rounded-xl text-sm font-medium text-white transition-colors flex items-center gap-2 bg-blue-600 hover:bg-blue-700 shadow-sm">
                ...
                {(!marksExamId || (marksExamId && marksExamSchedules.length === 0)) && marksExamId && (
                  <div className="text-center py-12 text-sm text-slate-400">No subject schedules found. Go to Schedule tab first.</div>
                )}

                {!marksExamId && (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                    <CheckCircle2 size={48} className="text-slate-300 mb-4" />
                    <h3 className="text-lg font-semibold text-slate-700 mb-1">Marks Entry Portal</h3>
              ...
              <motion.div key="results" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                    ...
                    {resultExamId && (
                      <>
                        <select value={resultClassFilter} onChange={e => { setResultClassFilter(e.target.value); setResultSectionFilter(''); }}
                          className="px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-full md:w-48">
                          <option value="">All Classes</option>
                        ...
                        <div className="relative">
                          ...
                          <input type="text" placeholder="Search student..." value={resultSearch} onChange={e => setResultSearch(e.target.value)}
                            className="pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 w-56" />
                        </div>
                      </>
                    )}
                  </div>
                  {resultExamId && (
                    <div className="flex gap-2">
                      <button onClick={handleGenerateResults} disabled={generating}
                        className={`px-4 py-2.5 rounded-xl text-sm font-medium text-white transition-colors flex items-center gap-2 ${generating ? 'bg-blue-400 cursor-wait' : 'bg-slate-800 hover:bg-slate-900 shadow-sm'}`}>
                          ...
                          <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-4 py-3">
                              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${
                                ...
                                r.meritPosition === 2 ? 'bg-slate-100 text-slate-600' :
                                r.meritPosition === 3 ? 'bg-orange-100 text-orange-700' :
                                'bg-slate-50 text-slate-500'
                              }`}>{r.meritPosition}</span>
                            </td>
                            <td className="px-4 py-3 font-medium text-slate-800">{r.studentName}<div className="text-xs text-slate-500">{r.studentId}</div></td>
                            <td className="px-4 py-3 text-slate-600">{r.className}</td>
                            <td className="px-4 py-3 font-semibold text-slate-800">{r.totalMarks}/{r.totalFullMarks}</td>
                            ...
                            <td className="px-4 py-3 text-right">
                              <button onClick={() => viewResultStudent(r.studentId)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View Details"><Eye size={16} /></button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                {!resultExamId && (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                    ...
                    <h3 className="text-lg font-semibold text-slate-700 mb-1">Exam Results</h3>
            ...
            {activeTab === 'reportcards' && (
              <motion.div key="reportcards" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                  ...
                  {reportExamId && (
                    <>
                      ...
                      <button onClick={handleBulkPrintReports} disabled={reportResults.length === 0}
                        className="px-4 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-sm font-medium shadow-sm transition-colors flex items-center gap-2">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {reportResults.map(r => (
                          <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-4 py-3 text-right">
                              <button onClick={() => handlePrintReport(r)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5 ml-auto">
                ...
                {reportExamId && reportResults.length === 0 && (
                  <div className="text-center py-12 text-sm text-slate-400">No results published for this exam. Publish results first.</div>
                )}

                {!reportExamId && (
                  <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                    <Printer size={48} className="text-slate-300 mb-4" />
                    <h3 className="text-lg font-semibold text-slate-700 mb-1">Report Cards</h3>
              ...
              <motion.div key="analytics" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                {analyticsData ? (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                        <h4 className="text-sm font-semibold text-slate-700 mb-3">Pass / Failure Rate</h4>
                        <div className="flex items-center gap-4">
                          <div className="flex-1">
                            <div className="flex justify-between text-sm mb-1">
                              <span className="text-emerald-600 font-medium">Pass Rate</span>
                              <span className="font-bold text-slate-800">{analyticsData.passRate}%</span>
                            </div>
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between text-sm mb-1">
                              ...
                              <span className="font-bold text-slate-800">{analyticsData.failureRate}%</span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2.5">
                      ...
                      <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                        <h4 className="text-sm font-semibold text-slate-700 mb-3">Class Performance</h4>
                        <div className="space-y-3">
                          {analyticsData.classPerformance.map(cp => (
                            <div key={cp.className}>
                              <div className="flex justify-between text-sm mb-1">
                                <span className="text-slate-700 font-medium">{cp.className}</span>
                                <span className="text-slate-600">{cp.averageGpa.toFixed(2)} GPA &bull; {cp.studentCount} students</span>
                              </div>
                              <div className="w-full bg-slate-100 rounded-full h-2">
                                <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${cp.passRate}%` }} />
                              </div>
                            </div>
                          ))}
                        </div>
                          ...
                          {analyticsData.topPerformers.slice(0, 5).map((p, i) => (
                            <div key={i} className="flex items-center justify-between py-1.5 border-b border-slate-100 last:border-0">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-slate-700">{p.studentName}</span>
                              </div>
                    ...
                    {/* Subject Performance */}
                    <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm">
                      <h4 className="text-sm font-semibold text-slate-700 mb-3">Subject-wise Performance</h4>
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                          <thead className="text-xs text-slate-500 uppercase bg-slate-50">
                            <tr>
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {analyticsData.subjectPerformance.map(sp => (
                              <tr key={sp.subjectName}>
                                <td className="px-4 py-2 font-medium text-slate-700">{sp.subjectName}</td>
                                <td className="px-4 py-2 text-slate-600">{sp.averageMarks.toFixed(1)}</td>
                                <td className="px-4 py-2 text-slate-600">{sp.fullMarks}</td>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-slate-500">
                    ...
                    <h3 className="text-lg font-semibold text-slate-700 mb-1">Exam Analytics</h3>
      ...
      <AnimatePresence>
        {examModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={closeExamModal} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl z-10 max-h-[90vh] overflow-y-auto">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10 rounded-t-2xl">
                <h3 className="font-bold text-slate-800 text-lg">{examModal.mode === 'add' ? 'Create New Exam' : 'Edit Exam'}</h3>
                <button onClick={closeExamModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><X size={20} /></button>
              </div>
              ...
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
                <button onClick={closeExamModal} className="flex-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-medium transition-colors">Cancel</button>
                <button onClick={handleSaveExam} disabled={examSaving}
                  className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium text-white transition-colors flex items-center justify-center gap-2 ${examSaving ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700 shadow-sm'}`}>
      ...
      <AnimatePresence>
        {scheduleModal.open && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={closeScheduleModal} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg z-10">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 text-lg">{scheduleModal.mode === 'add' ? 'Add' : 'Edit'} Schedule</h3>
                <button onClick={closeScheduleModal} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><X size={20} /></button>
              </div>
              <div className="p-6 space-y-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={scheduleModal.data.isPractical || false} onChange={e => handleScheduleField('isPractical', e.target.checked)}
                    className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                  <span className="text-sm text-slate-700">Practical Examination</span>
                </label>

                {/* Conflict Check */}
                <div className="pt-2">
                  <button onClick={checkConflicts} className="px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl text-sm font-medium transition-colors flex items-center gap-2">
                    <AlertTriangle size={15} /> Check Conflicts
                  </button>
                  {scheduleConflicts.length > 0 && (
                    <div className="mt-3 p-3 bg-rose-50 border border-rose-200 rounded-xl">
                      <p className="text-xs font-semibold text-rose-700 mb-1.5">{scheduleConflicts.length} Conflict(s) Detected:</p>
              ...
              <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
                ...
                <button onClick={closeScheduleModal} className={`${scheduleModal.mode === 'edit' ? 'flex-1' : 'flex-1'} px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-medium transition-colors`}>Cancel</button>
                <button onClick={handleSaveSchedule} disabled={scheduleSaving}
                  className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium text-white transition-colors flex items-center justify-center gap-2 ${scheduleSaving ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700 shadow-sm'}`}>
      ...
      <AnimatePresence>
        {viewingResult && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={() => setViewingResult(null)} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-3xl z-10 max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-white z-10 px-6 py-4 border-b border-slate-100 flex items-center justify-between rounded-t-2xl">
                <h3 className="font-bold text-slate-800 text-lg">Result Details</h3>
                <div className="flex items-center gap-2">
                  <button onClick={() => handlePrintReport(viewingResult)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium transition-colors flex items-center gap-1.5">
                    <Printer size={14} /> Print
                  </button>
                  <button onClick={() => setViewingResult(null)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"><X size={20} /></button>
                </div>
              </div>
              <div className="p-6">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                  <div>
                    <h2 className="text-xl font-bold text-slate-800">{viewingResult.studentName}</h2>
                    <p className="text-sm text-slate-500">{viewingResult.studentId} &bull; {viewingResult.className} &bull; {viewingResult.section}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-bold text-slate-800">{viewingResult.gpa.toFixed(2)}</div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {viewingResult.subjectResults.map(sr => (
                      <tr key={sr.subjectId}>
                        <td className="px-4 py-2.5 font-medium text-slate-700">{sr.subjectName}</td>
                        <td className="px-4 py-2.5 text-center text-slate-600">{sr.fullMarks}</td>
                        <td className="px-4 py-2.5 text-center text-slate-600">{sr.passMarks}</td>
                  ...
                  <tfoot className="bg-slate-50 font-semibold">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-center text-slate-700">{viewingResult.totalFullMarks}</td>
                      <td className="px-4 py-3 text-center text-slate-700"></td>
                      <td className="px-4 py-3 text-center text-slate-800">{viewingResult.totalMarks}</td>
                      ...
                      <td className="px-4 py-3 text-center text-slate-800">{viewingResult.gpa.toFixed(2)}</td>
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-sm z-10 p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-2">Delete {confirmDelete.type === 'exam' ? 'Exam' : 'Schedule'}?</h3>
              <p className="text-sm text-slate-500 mb-6">
                Are you sure you want to delete <span className="font-semibold text-slate-700">{confirmDelete.label}</span>? This will also remove associated schedules and marks.
              </p>
...
function StatCard({ label, value, color }: { label: string; value: string; color: string }) {
  const colorBar: Record<string, string> = { blue: 'bg-blue-500', indigo: 'bg-indigo-500', emerald: 'bg-emerald-500', rose: 'bg-rose-500', amber: 'bg-amber-500' };
  return (
    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
      <div className={`w-1.5 h-12 rounded-full ${colorBar[color] || 'bg-slate-500'}`} />
      <div>
